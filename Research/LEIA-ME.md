...
Projeto LaTeX do artigo **"Diagnóstico de defeitos de impressão em etiquetas de
códigos de barras: uma abordagem por visão computacional e orquestração
multiagente"**, no formato de **artigo científico** do abnTeX2 (conforme ABNT).

> **✅ Compilação já testada** (pdfLaTeX + BibTeX, TeX Live 2026): **27 páginas, sem
> erros**, com sumário, citações e referências resolvidos e fontes embutidas. Um PDF
> de exemplo está em `diagnostico-de-defeitos-de-impressao-em-etiquetas-de-codigos-de-barras.pdf`. No Overleaf o resultado é o mesmo.
>
...
|---|---|---|
| `diagnostico-de-defeitos-de-impressao-em-etiquetas-de-codigos-de-barras.tex` | Texto completo do artigo (todo o conteúdo está aqui) | **Sim** |
...
2. Faça o *upload* do arquivo **`diagnostico-de-defeitos-de-impressao-em-etiquetas-de-codigos-de-barras-overleaf.zip`** (que está na pasta acima
   ...
   *Alternativa:* crie um projeto em branco e arraste para dentro dele os arquivos
   `diagnostico-de-defeitos-de-impressao-em-etiquetas-de-codigos-de-barras.tex`, `referencias.bib` e a pasta `figuras/`.
3. Abra **Menu** (canto superior esquerdo) e configure:
   - **Compiler / Compilador:** `pdfLaTeX`
   - **TeX Live version:** `2024` (ou a mais recente)
   - **Main document:** `diagnostico-de-defeitos-de-impressao-em-etiquetas-de-codigos-de-barras.tex`
...
## O que você precisa preencher (procure por `[...]` no `diagnostico-de-defeitos-de-impressao-em-etiquetas-de-codigos-de-barras.tex`)

- **Título** (topo do `diagnostico-de-defeitos-de-impressao-em-etiquetas-de-codigos-de-barras.tex`): `\titulo{...}` (português) e
...
O `diagnostico-de-defeitos-de-impressao-em-etiquetas-de-codigos-de-barras.tex` traz o código de referência (agentes ADK + FastAPI) em blocos de
