...
\subsection{Comparação explícita com o estado da arte}\label{sec-sota}
...
repositório (\texttt{experimentos.py}). Um quarto experimento, \textbf{(E3)},
...
quantificar esse ganho, compara-se a classificação \textbf{CNN-pura}
(Tabela~\ref{tab-confusao}) com a classificação \textbf{pós-arbitragem} do agente
...
acertos/erros das duas condições nas 30 etiquetas dessas duas classes (15
\emph{sem defeito} $+$ 15 \emph{cabeça queimada}). Como o árbitro multimodal
depende de uma chave de API do Gemini, ausente no ambiente de avaliação, os números
de E3 ficam registrados como \textbf{[PLACEHOLDER --- a preencher quando houver
chave Gemini]}: acurácia CNN-pura no par, acurácia pós-arbitragem, contagens
discordantes $b/c$ e $p$-valor de McNemar. O protocolo, contudo, já está definido e
é executável assim que a chave estiver disponível.
...
público \textbf{BarBeR}, reportando acurácia e Macro-F1 no domínio real, além da
matriz de confusão por classe. Espera-se queda frente ao teste em distribuição;
ainda assim, o experimento mede a robustez e aproxima a avaliação do estado da
arte. Os números ficam como \textbf{[PLACEHOLDER --- a preencher após a execução
...
antes de emitir a causa (o ganho quantitativo é o experimento E3, cujos números
dependem de chave Gemini --- Seção~\ref{sec-baseline}). Vale notar que a
...
ganho ainda careça de quantificação (experimento E3, Seção~\ref{sec-baseline});
...
recurso que a abordagem monolítica não tem (a quantificação do ganho fica pendente de
chave de LLM).
...
executar o experimento E3 (ganho da arbitragem visual multimodal sobre a CNN-pura no
par confundível) e a comparação quantitativa multiagente \textit{versus} monolítica na
qualidade do diagnóstico, ambos com chave de LLM e causas de referência rotuladas;
